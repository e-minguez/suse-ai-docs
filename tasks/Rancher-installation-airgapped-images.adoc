...
This section describes how to set up your private registry so that when you install {ranchermanager}, it will pull all the required images from this registry.
By default, all images used to link:https://rancher.com/docs/rancher/v2.x/en/cluster-provisioning/[provision {k8s} clusters] or launch any tools in {ranchermanager}, e.g., monitoring, pipelines or alerts, are pulled from {dhub}.
In an air-gapped installation of {ranchermanager}, you will need a private registry that is located somewhere accessible by your {ranchermanager} server.
...
. From the release's *Assets* section, download the following files, which are required to install {ranchermanager} in an air-gapped environment:
...
If you are upgrading {ranchermanager} and using a version of {certmanager} older than v0.12.0, please see our link:https://rancher.com/docs/rancher/v2.x/en/installation/resources/upgrade-cert-manager/[upgrade documentation].
====
+
[source]
----
