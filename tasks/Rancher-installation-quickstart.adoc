// source: https://github.com/rancher/rancher-product-docs/edit/main/versions/v2.13/modules/en/pages/installation-and-upgrade/install-rancher.adoc
// base URL: https://documentation.suse.com/cloudnative/rancher-manager/latest/en/
// product name: {ranchera}
...
* *K3s:* For the tutorial to install a K3s Kubernetes cluster, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/install-kubernetes/k3s-for-rancher.html[this page.] For help setting up the infrastructure for a high-availability K3s cluster, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/infrastructure-setup/ha-k3s-kubernetes-cluster.html[this page.]
* *RKE2:* For the tutorial to install an RKE2 Kubernetes cluster, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/install-kubernetes/rke2-for-rancher.html[this page.] For help setting up the infrastructure for a high-availability RKE2 cluster, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/infrastructure-setup/ha-rke2-kubernetes-cluster.html[this page.]
* *Amazon EKS:* For details on how to install Rancher on Amazon EKS, including how to install an Ingress controller so that the Rancher server can be accessed, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/hosted-kubernetes/rancher-on-amazon-eks.html[this page.]
* *AKS:* For details on how to install Rancher with Azure Kubernetes Service, including how to install an Ingress controller so that the Rancher server can be accessed, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/hosted-kubernetes/rancher-on-aks.html[this page.]
* *GKE:* For details on how to install Rancher with Google Kubernetes Engine, including how to install an Ingress controller so that the Rancher server can be accessed, refer to link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/hosted-kubernetes/rancher-on-gke.html[this page.] GKE has two modes of operation when creating a Kubernetes cluster, Autopilot and Standard mode. The cluster configuration for Autopilot mode has restrictions on editing the kube-system namespace. However, Rancher needs to create resources in the kube-system namespace during installation. As a result, you will not be able to install Rancher on a GKE cluster created in Autopilot mode.
...
* https://docs.helm.sh/using_helm/#installing-helm[helm] - Package management for Kubernetes. Refer to the link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/requirements/helm-version-requirements.html[Helm version requirements] to choose a version of Helm to install Rancher. Refer to the https://helm.sh/docs/intro/install/[instructions provided by the Helm project] for your specific platform.
...
For systems without direct internet access, see link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/other-installation-methods/air-gapped/install-rancher-ha.html[Air Gap: Kubernetes install].

To choose a Rancher version to install, refer to xref:[Choosing a Rancher Version.]

To choose a version of Helm to install Rancher with, refer to the link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/requirements/helm-version-requirements.html[Helm version requirements]
...
If you want to externally terminate SSL/TLS, see link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/references/helm-chart-options.html#_external_tls_termination[TLS termination on an External Load Balancer]. As outlined on that page, this option does have additional requirements for TLS verification.
====


There are three recommended options for the source of the certificate used for TLS termination at the Rancher server:

* *Rancher-generated TLS certificate:* In this case, you will need to install `cert-manager` into the cluster. Rancher utilizes `cert-manager` to issue and maintain its certificates. Rancher will generate a CA certificate of its own, and sign a cert using that CA. `cert-manager` is then responsible for managing that certificate. No extra action is needed when `agent-tls-mode` is set to strict. More information can be found on this setting in link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/references/tls-settings.html#_agent_tls_enforcement[Agent TLS Enforcement].
* *Let's Encrypt:* The Let's Encrypt option also uses `cert-manager`. However, in this case, cert-manager is combined with a special Issuer for Let's Encrypt that performs all actions (including request and validation) necessary for getting a Let's Encrypt issued cert. This configuration uses HTTP validation (`HTTP-01`), so the load balancer must have a public DNS record and be accessible from the internet. When setting `agent-tls-mode` to `strict`, you must also specify `--privateCA=true` and upload the Let's Encrypt CA as described in link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/resources/tls-secrets.html[Adding TLS Secrets]. More information can be found on this setting in link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/references/tls-settings.html#_agent_tls_enforcement[Agent TLS Enforcement].
...
____
You should skip this step if you are bringing your own certificate files (option `ingress.tls.source=secret`), or if you use link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/references/helm-chart-options.html#_external_tls_termination[TLS termination on an external load balancer].
____
...
----
helm install rancher rancher-prime/rancher-prime \
...
When `agent-tls-mode` is set to `strict` (the default value for new installs of Rancher starting from v2.9.0), you must supply the `privateCA=true` chart value (e.x. through `--set privateCA=true`) and upload the Let's Encrypt Certificate Authority as outlined in link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/resources/tls-secrets.html[Adding TLS Secrets]. Information on identifying the Let's Encrypt Root CA can be found in the Let's Encrypt https://letsencrypt.org/certificates/[docs]. If you don't upload the CA, then Rancher may fail to connect to new or existing downstream clusters.
====
...
If you want to check if your certificates are correct, see link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/faq/technical-items.html#_how_do_i_check_common_name_and_subject_alternative_names_in_my_server_certificate[How do I check Common Name and Subject Alternative Names in my server certificate?]
====
...
Now that Rancher is deployed, see link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/resources/tls-secrets.html[Adding TLS Secrets] to publish the certificate files so Rancher and the Ingress controller can use them.
--
======
...
* link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/references/helm-chart-options.html#_http_proxy[HTTP Proxy]
* link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/references/helm-chart-options.html#_private_registry_and_air_gap_installs[Private Container Image Registry]
* link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/references/helm-chart-options.html#_external_tls_termination[TLS Termination on an External Load Balancer]

See the link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/references/helm-chart-options.html[Chart Options] for the full list of options.
...
Doesn't work? Take a look at the link:https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/troubleshooting/troubleshooting.html[Troubleshooting] Page