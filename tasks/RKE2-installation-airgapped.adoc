...
{rke2} can be installed in an air-gapped environment with two different methods. You can either deploy via the `rke2-airgap-images` tarball artifact or by using a private registry.
endif::[]
...
. <<Install RKE2>> using the `system-default-registry` parameter, or use the link:https://documentation.suse.com/cloudnative/rke2/latest/en/install/containerd_registry_configuration.html[containerd registry configuration] to use your registry as a mirror for docker.io.
